 ...
 *
 * Server-side proxy upload: client sends raw file bytes, server stores to GCS.
 * Bypasses CORS restrictions on presigned URLs.
 */
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await file.save(rawBody, {
        metadata: { contentType, originalName: fileName },
        resumable: false,
      });

      const objectPath = `/objects/uploads/${objectId}`;
      res.json({ objectPath, metadata: { name: fileName, contentType } });
    } catch (error) {
